...
// ── Naming convention for all future image drops ─────────────────────────────
...
// This ensures files sort correctly and never clash across years
...
// ── Album definitions — featured albums shown above the main grid ────────────
const featuredAlbums = [
  {
    key: "Beauty Contest 2026" as Category,
  ...
  {
    key: "Athletics Day 2026" as Category,
...
export default function GalleryPage() {
  ...
  const featuredKeys = new Set(featuredAlbums.map((a) => a.key));
  ...
  return (
    <>
        ...
        <div className="max-w-6xl mx-auto">
          <FadeIn>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3" style={{ gridAutoRows: "200px" }}>
              ...
              <div className="rounded-xl overflow-hidden">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={`${BASE}/athletics-day-20260610-3.jpeg`} alt="Athletics Day 2026" className="w-full h-full object-cover hover:scale-105 transition-transform duration-700" />
              </div>
            </div>
          </FadeIn>
        </div>
      ...
      {featuredAlbums.map((album) => (
        <AlbumSection key={album.key} albumKey={album.key} title={album.title} date={album.date} description={album.description} />
      ))}
